...
> Status: **Draft for review (rev 6)** · Author: Dylan (with Claude) · Date: 2026-06-06
...
**Record worktree ownership.** When the *new worktree* path is taken, the agent tab is stamped as **owning a cmux-created worktree** (its path + branch), persisted on the tab. The *current pwd/branch* path stamps **no ownership** — cmux didn't create that directory and must never remove it. This flag is the sole input to close-time cleanup (§4.10).
...
- **Worktree cleanup (conditional, §4.9 flag):**
  - Tab **owns a cmux-created worktree** → on close, `git worktree remove` it (and prune the branch only if it was the cmux-created one and is merged/empty). **Guard first:** if the worktree has uncommitted changes or unpushed commits, do **not** auto-remove — confirm with the user and default to *keep the worktree* (surface the dirty/unpushed state). Clean worktree → remove (with the standard close confirm).
  - Tab launched in an **existing pwd/branch** (no ownership flag) → close the tab only; **touch no files**.
  - Closing a **chat room** cascades to its group; each tab applies the above. Batch the worktree confirmations (one summary listing dirty/unpushed worktrees) rather than N dialogs.
...
- **Worktree ownership** (§4.9/§4.10): persist whether the agent tab owns a cmux-created worktree + its path/branch, so close-time cleanup works after a restart. Absent ⇒ no ownership ⇒ no cleanup (safe default for migrated/legacy tabs).
...
- **Worktree cleanup** (through an injected `WorktreeManaging` seam — fakeable, no real git): closing a tab that owns a **clean** cmux-created worktree calls remove; **dirty/unpushed** → remove is NOT called, user prompted, default keep; a tab **without** the ownership flag → remove never called (no filesystem touch); closing a room batches its worktree confirmations.
...
14b. **Worktree cleanup:** close an agent created in a *new worktree* (clean) → its worktree is removed. Close one with **uncommitted/unpushed** work → prompted, worktree **kept** by default. Close an agent launched in the *current pwd/branch* → tab closes, **no files touched**.
