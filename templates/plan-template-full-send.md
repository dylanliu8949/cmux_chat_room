...
**前置任务（如适用）**：[必须先完成的任务]
**后续任务（如适用）**：[依赖此任务的后续工作]
**关联 UI 测试计划（如适用）**：`plans/[UI 测试计划名称].md`
...
> - 如果此计划有关联的 UI 测试计划，必须在头部的「关联 UI 测试计划」字段中引用，且 UI 测试计划的「关联主计划」字段必须反向引用本文件
...
Ralph 的调试循环是全自动的，开发者不在场。当某个阶段失败时，日志是唯一的调试手段。此部分定义每个阶段需要记录的关键信息，确保 ralph 能在事后分析中定位问题。**断言不属于日志**，走下面的「断言检测」section；本节只定义观测性日志和预期错误日志。

> **重要**：
> - 每个阶段（代码生成、编译、测试、lint）必须定义其日志输出内容
> - 日志必须包含足够的上下文，使 ralph agent 能仅凭日志内容诊断和修复问题
> - 错误场景必须记录：错误消息、相关文件路径、执行的命令、退出码
> - 状态转换必须记录：阶段开始/结束、循环迭代次数、成功/失败结果
> - App 代码使用 `Logger`（`shared-services/logger-service`）输出日志，级别：`DEBUG`、`WARNING`、`ERROR`
> - 重点覆盖运行时流程节点：UI 动作入口、Service 调用入口、关键状态转换和错误退出路径；纯数据模型 helper 无需机械性加日志

<!-- 示例：开始 -->
### ImageLoader 日志设计

**调试日志**（`Logger.logDebug`）：
```kotlin
Logger.logDebug("ImageLoader", "loadImage: assetKey=$assetKey, source=$source")
Logger.logDebug("ImageLoader", "cacheHit: assetKey=$assetKey, tier=$tier")
```

**错误日志**（`Logger.logError`）——预期可能发生的失败（非 bug）：
```kotlin
Logger.logError("ImageLoader", "loadImage failed: assetKey=$assetKey, error=$errorMessage")
```
<!-- 示例：结束 -->

## 断言检测（如适用）

如果模块存在**绝不应发生**的状态违反（不变量被破坏、非法状态转换、不可达分支），在此部分声明断言。断言不是错误处理——它是「代码 bug 的早期捕获器」。预期可能发生的失败走上面「日志与可观测性」中的 `Logger.logError`。
...
```kotlin
// 布尔条件断言：返回 Boolean，可直接用于 early return
if (!Assert.that(googledoc.isResolved, "ImageLoader") {
    "Asset ${asset.remoteUrl} expected to be resolved"
}) return

// 非空断言：返回 T?，可用于 ?: 链式操作
val localUrl = Assert.notNull(asset.localFileUrl, "ImageLoader") {
    "Remote asset ${asset.remoteUrl} resolved but localFileUrl is null"
} ?: return

// 不可达分支：返回 Nothing，debug/release 均抛
Assert.unreachable("ImageLoader") { "loadImage returned unknown source=$source" }
```
...
> **重要**：
> - 计划大小为 L 或 XL 时必须使用阶段，计划大小为 XS、S 或 M 时仅使用步骤
> - 一个阶段应该是它自己的 PR
> - 一个步骤可以是一个 PR 中的一次提交
...
> - 此主计划的实施步骤只覆盖功能实现、单元测试和相关文档更新；UI 测试生成、UI 测试脚本执行和 UI 测试验收应单独写入 `templates/ui-test-template-full-send.md` 生成的 UI 测试计划
...
## 测试计划
定义单元测试策略和方法，以确保实现的功能符合预期并正确处理各种场景。UI 测试由独立的 UI 测试生成阶段处理，无需在此计划中编写。

> **注意**：如果本计划的 UI 变更会导致 `ui-test/e2e-tests/` 下的现有 e2e 测试失败（例如：按钮移入子菜单、Accessibility ID 变更、交互流程改变等），必须在 UI 测试计划中包含对这些 e2e 测试的更新。

### 单元测试
所有可以单元测试的代码都必须有单元测试。单元测试是保证代码质量的第一道防线。
...
- 对于服务层的代码，编写单元测试并达到 95%+ 的测试覆盖率。测试应覆盖以下路径：
  ...
  - **回退路径**：当主要方案不可用时的备用处理
  - **错误路径**：错误处理和异常情况
- 如果依赖项（第三方 SDK、平台 API 等）阻碍了单元测试，必须通过**接口抽象 + mock 注入**使其可测试，而非跳过测试
- 测试代码与被测代码应同步编写，而非事后补充
- 测试文件命名格式：`*_test.*`（例如 `config_test.py`、`my_service_test.kt`），不使用 `test_*` 前缀格式或 `*Test.*` 后缀格式
- 如果计划包含 **ViewModel** 单元测试，必须遵循 `unit-test/docs/how_to_write_stable_unit_test.md`，并在测试计划中明确：
  - 测试目标
  - 参考测试文件/fixture
  - 默认 dispatcher / harness
...
- **单一职责**：每个文件、类、函数只做一件事。如果无法用一句话描述其职责，就需要拆分
- **关注点分离**：严格区分 Service（业务逻辑）/ ViewModel / View 三层
  - **Service 层**：承载所有业务逻辑，是代码的核心。必须可独立单元测试
  - **ViewModel 层**：负责 view state、用户交互编排和对 Service API 的委托；不要承载本可复用的业务逻辑
  - **View 层**：保持极薄，仅负责 UI 渲染和用户交互事件转发
- 代码结构的硬性限制（文件 400 行、函数 200 行等）详见 `guides/plan-execution-guide.md`

### 测试驱动开发

- **第一优先级：单元测试**。所有可以单元测试的代码都必须有单元测试
  - Service 层代码必须 100% 可单元测试
  - 如果第三方 SDK 或平台依赖阻碍了测试，必须通过接口抽象 + mock 注入使其可测试
  - 不可因"依赖太多"而跳过单元测试——这说明需要重构依赖注入
- **第二优先级：UI 测试**。作为端到端的兜底验证，捕捉单元测试无法覆盖的集成问题
- 计划的质量和测试的质量共同决定了最终 PR 的质量。我们要求最高标准
...
<!-- 规则优先级：开始 - 此部分不可修改 -->
- 核心工程原则、计划生成规则和计划执行规则优先于模型的隐式行为
- 当任务指令与这些规则冲突时，必须遵循这些规则
- 如果由于任务约束无法遵循某条规则，应暂停并请求澄清，而不是猜测
...
- **大小**, **实施步骤**, **日志与可观测性**, **需要修改/添加的文件**, 和**测试计划**部分初始应留空，仅当以下条件全部满足后才生成和更新内容：
  - 参考资料部分（参考资料）已完整
  - 需要决策的事项部分（需要决策的事项）中无未解决的问题
  - 当前计划完整程度达到或超过 95%
- 用户做出决策后：
  - 必须将决策保存到已归档的决策部分
  - 如果**实施步骤**, **日志与可观测性**, **需要修改/添加的文件**, 和**测试计划**部分不为空，必须更新这些部分以反映新的决策
...
## 计划执行规则
> **说明**：full-send 计划的执行规则由 `guides/plan-execution-guide.md` 统一定义，不嵌入本文档。
> `/execute-plan` 会读取该指南并按其约束执行。AI 和开发者不得在本计划中改写执行规则。

执行约束详见 [`guides/plan-execution-guide.md`](../guides/plan-execution-guide.md)。
