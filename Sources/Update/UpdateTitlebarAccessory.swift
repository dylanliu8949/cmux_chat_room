...
/// Shared styling for header-chrome icon controls (e.g. titlebar update pill,
/// minimal-mode sidebar controls). Previously colocated with the now-removed
/// right-sidebar chrome; kept here since the surviving header-chrome consumers
/// still depend on it.
...
struct TitlebarControlsView: View {
    ...
    private func controlsGroup(config: TitlebarControlsStyleConfig, foregroundColor: Color) -> some View {
        ...
        let content = HStack(spacing: config.spacing) {
            ...
            // Chat-room fork: no global "new workspace" button — agents/rooms are created from the
            // sidebar section "+" controls only (there are no plain non-agent terminals).
