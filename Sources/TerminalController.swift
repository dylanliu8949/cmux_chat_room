...
class TerminalController {
    ...
    nonisolated enum PortScanKickReason: String {
        case command
        case refresh
    }
            ...
            v2MainSync {
                ...
                if let ws = callerTabManager.tabs.first(where: { $0.id == wsId }) {
                    ...
                    if let surfaceId, ws.panels[surfaceId] != nil {
                        ...
                        payload["is_browser_surface"] = false
    ...
    private func readTerminalTextFromVTExportForSnapshot(
        ...
        #if DEBUG
        cmuxDebugLog("snapshot.vtExport action=\(bindingAction) succeeded=\(actionSucceeded) hasPath=\(exportedPath != nil)")
        #endif
    ...
    private func orderedPanels(in tab: Workspace) -> [any Panel] {
        ...
        // serializer and the reorder gate consistent.
